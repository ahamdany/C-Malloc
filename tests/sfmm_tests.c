...
Test(sf_memsuite_student, Malloc_an_Integer, .init = sf_mem_init, .fini = sf_mem_fini){
  int *x = sf_malloc(sizeof(int));
  *x = 4;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
}

Test(sf_memsuite_student, Malloc_OVER_SIZE, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*5);
  cr_assert_null(x, "x is not NULL!");
  cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");

  }

  Test(sf_memsuite_student, Malloc_OVER_SIZE1, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*7);
  cr_assert_null(x, "x is not NULL!");
  cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");

  }

  Test(sf_memsuite_student, Malloc_under_SIZE2, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(0);
  cr_assert_null(x, "x is not NULL!");
  cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");
  ...
  Test(sf_memsuite_student, Malloc_EXACT_SIZE, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;

  int *x = sf_malloc((PAGE_SZ*4)-16);
  cr_assert_null(x, "x is not NULL!");
  cr_assert(sf_errno == ENOMEM, "sf_errno is not ENOMEM!");

  }

  Test(sf_memsuite_student, Malloc_double_malloc, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(sizeof(int));
  ...
  *y =5;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
  cr_assert(*y == 5, "Failed to properly sf_malloc space for an integer!");

 }

 Test(sf_memsuite_student, Malloc_triple_malloc, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(sizeof(int));
  int *y = sf_malloc(sizeof(int));
  ...
  *z =6;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
  cr_assert(*y == 5, "Failed to properly sf_malloc space for an integer!");
  cr_assert(*z == 6, "Failed to properly sf_malloc space for an integer!");
}

 Test(sf_memsuite_student, Malloc_exactly_1_pages, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*1);
  cr_assert_not_null(x);
  *x = 4;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
  cr_assert(sf_errno == 0, "sf_errno is not ZERO!");
 ...
 Test(sf_memsuite_student, Malloc_exactly_2_pages, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*2);
  cr_assert_not_null(x);
  *x = 4;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
  cr_assert(sf_errno == 0, "sf_errno is not ZERO!");
 ...
 Test(sf_memsuite_student, Malloc_exactly_3_pages, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*3);
  cr_assert_not_null(x);
  *x = 4;
  cr_assert(*x == 4, "Failed to properly sf_malloc space for an integer!");
  cr_assert(sf_errno == 0, "sf_errno is not ZERO!");
...
Test(sf_memsuite_student, Malloc_exactly_4_pages, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*4);
  cr_assert_null(x);
  cr_assert(sf_errno == ENOMEM, "sf_errno is not ENOMEM!");
}

Test(sf_memsuite_student, Malloc_exactly_6_pages_increment, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*1);
  int *y = sf_malloc(PAGE_SZ*2);
  int *z = sf_malloc(PAGE_SZ*3);

  cr_assert_not_null(x);
  cr_assert_not_null(y);
  cr_assert_null(z);
  cr_assert(sf_errno == ENOMEM, "sf_errno is not zero!");
}

Test(sf_memsuite_student, Malloc_exactly_6_pages_increment_value, .init = sf_mem_init, .fini = sf_mem_fini){
  sf_errno = 0;
  int *x = sf_malloc(PAGE_SZ*1);
  int *y = sf_malloc(PAGE_SZ*2);
  int *z = sf_malloc(PAGE_SZ*3);
  ...
  *y = 6000;
  ...
  cr_assert(sf_errno == ENOMEM, "sf_errno is not zero!");
  cr_assert(*x == 4, "x is not 4!");
  cr_assert(*y == 6000, "y is not 6000!");
  cr_assert_null(z);
}

Test(sf_memsuite_student, Malloc_3_pages_plus, .init = sf_mem_init, .fini = sf_mem_fini){
  int *x = sf_malloc(PAGE_SZ*3 + 4000);
  cr_assert_not_null(x);
  cr_assert(sf_errno == 0, "sf_errno is not ENOMEM!");
}


Test(sf_memsuite_student, Malloc_invalid_then_valid, .init = sf_mem_init, .fini = sf_mem_fini){
  int *x = sf_malloc(PAGE_SZ*4);
  int *y = sf_malloc(PAGE_SZ*2);
...
Test(sf_memsuite_student, free_triple_free, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;
    void *x = sf_malloc(sizeof(int));
    sf_free(x);
    sf_free(x);
...
Test(sf_memsuite_student, free_friple_freee, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;
    void *x = sf_malloc(sizeof(int));
    sf_free(x);
    sf_free(x);
...
Test(sf_memsuite_student, free_invalid_ptr_after, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;
    char* y = 0;
    void *x = sf_malloc(sizeof(int));
    (void)x;
...
Test(sf_memsuite_student, free_invalid_ptr_before, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;
    char* y = 0;
    void *x = sf_malloc(sizeof(int));
    (void)x;
...
Test(sf_memsuite_student, free_invalid_ptr_null, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;
    void *x = sf_malloc(sizeof(int));
    (void)x;
    sf_free(NULL);
...
Test(sf_memsuite_student, realloc_size_0, .init = sf_mem_init, .fini = sf_mem_fini) {
    void *x = sf_malloc(sizeof(int) * 8);
    void *y = sf_realloc(x,0);

    cr_assert_null(y, "y is not NULL!");
    free_list *fl = &seg_free_list[find_list_index_from_size(4016)];

    cr_assert_not_null(fl->head, "No block in expected free list!");
    cr_assert(fl->head->header.allocated == 0, "Allocated bit is set!");
    cr_assert(fl->head->header.block_size << 4 == 4096, "Free block size was %d should be %d!",fl->head->header.block_size << 4,4016);
}
